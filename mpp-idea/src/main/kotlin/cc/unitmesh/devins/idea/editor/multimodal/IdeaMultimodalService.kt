...
class IdeaMultimodalService(private val project: Project) : Disposable {
    ...
    private val initializationStarted = java.util.concurrent.atomic.AtomicBoolean(false)
    ...
    fun initialize() {
        ...
        // Use compareAndSet to ensure only one initialization runs
        if (!initializationStarted.compareAndSet(false, true)) {
            return // Another thread is already initializing
        }
        
        // Run initialization on pooled thread to avoid blocking any thread
