...
/**
 * JavaScript code to inject into JBCefBrowser for DOM selection and communication
 *
 * This script provides:
 * - Selection mode with element highlighting
 * - DOM tree extraction
 * - Bidirectional communication with Kotlin via JBCefJSQuery
 */
object IdeaWebEditBridgeScript {

    /**
     * Get the bridge script with the callback function name injected
     */
    fun getScript(callbackFunctionName: String): String = """
(function() {
    ...
    style.textContent = ':root { --webedit-hover-color: #2196F3; --webedit-selected-color: #4CAF50; }';
    ...
    window.webEditBridge = {
        ...
        getDOMTree: function() {
            ...
            function buildTree(el, depth) {
                ...
                var childElements = Array.from(el.children).slice(0, 20);
                for (var i = 0; i < childElements.length; i++) {
                    var childNode = buildTree(childElements[i], depth + 1);
        ...
        // Send message to Kotlin via JBCefJSQuery
        sendToKotlin: function(type, data) {
            try {
                var message = JSON.stringify({ type: type, data: data });
                $callbackFunctionName(message);
            } catch(e) {
                console.error('[WebEditBridge] sendToKotlin error:', e);
            }
        }
    };
