...
actual object ConfigManager {
    ...
    actual suspend fun load(): AutoDevConfigWrapper {
        return try {
            ...
            if (content == null) {
                logger.info("WASM: No config found in localStorage, returning empty config")
                return createEmpty()
            }
            ...
            logger.error("WASM: Error loading config: ${e.message}")
            createEmpty()
        }
    }
    ...
    actual suspend fun save(configFile: ConfigFile) {
        try {
            ...
            logger.info("WASM: Config saved to localStorage")
        } catch (e: Exception) {
            logger.error("WASM: Error saving config: ${e.message}")
            throw e
        }
    }
    ...
    actual suspend fun loadToolConfig(): ToolConfigFile {
        return try {
            ...
            if (content == null) {
                logger.info("WASM: No tool config found in localStorage, returning default")
                return ToolConfigFile.default()
            }

            json.decodeFromString<ToolConfigFile>(content)
        } catch (e: Exception) {
            logger.error("WASM: Error loading tool config: ${e.message}")
    ...
    actual suspend fun saveToolConfig(toolConfig: ToolConfigFile) {
        try {
            ...
            logger.info("WASM: Tool config saved to localStorage")
        } catch (e: Exception) {
            logger.error("WASM: Error saving tool config: ${e.message}")
            throw e
        }
    }
